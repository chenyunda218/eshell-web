    ...
    query: {
      bool: {
        ...
        filter: [
          ...
          {
            match_phrase: {
              "data_stream.namespace": "boardware",
            },
          },
          {
            match_phrase: {
...
const SignatureSource: React.FC = () => {
  const [data, setData] = React.useState<SignatureSourceProps[]>([]);
  const [isLoading, setIsLoading] = React.useState<boolean>(false);
  const date: SuperDatePicker = useSelector((state: any) => state.date);
  useEffect(() => {
    setIsLoading(true);
    query(date.start, date.end).then((res: any) => {
      let d: SignatureSourceProps[] = [];
      const sourceIps = res.aggregations.sourceIp.buckets;
      setIsLoading(false);
      if (sourceIps.length === 0) {
        setData([]);
        return;
      }
      sourceIps.forEach((sourceIp: any) => {
        sourceIp.destinationIp.buckets.forEach((destinationIp: any) => {
          destinationIp.signature.buckets.forEach((signature: any) => {
            signature.sourcetype.buckets.forEach((sourcetype: any) => {
              d.push({
                sourceIp: sourceIp.key,
                destinationIp: destinationIp.key,
                signature: signature.key,
                sourcetype: sourcetype.key,
                count: sourcetype.doc_count,
              });
            });
          });
        });
...
export default SignatureSource;
